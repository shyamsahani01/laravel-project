                        ...
                        <table id="datatable" class="table table-striped table-bordered table-responsive" style="width:100%">
                          <thead>
                            ...
                            <tr style="text-align: center;">
                              ...
                              <th style="text-align: center; min-width: 80px;">
                                <a href="javascript:void(0)" title="Total Sallery Of Unit 1 Employees + 1,50,000" class="table-heading-a" >MANPOWER COST </th>
                          ...
                          <tbody>
                            ...
                            @foreach($fgData as $key => $data)
                              @php
                                ...
                                // $manpower_cost = ( round($data->employee_data->per_day_sallery) > 0 ) ? ( round($data->employee_data->per_day_sallery) + 150000 ) : 0 ;
                            ...
                            <tr>
                              ...
                              <td style="text-align: center;">
                                ...
                                <a href="/fg/unit1/details?fg_date={{ date('Y-m-d',strtotime($data->FdDt)) }}" target="_blank"  style="color: green;">{{ date('d-m-Y',strtotime($data->FdDt)) }} </a>
                              </td>
                              <td style="text-align: center;"> {{ $No_OF_PROD_WORKERS_PRESENT_TODAY }} </td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($TODAY_SALARY_OF_PRESENT_PROD_WORKERS, 2)  }} </td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($direct_production_gw_amount, 2) }} </td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($total_direct_production_cost_amount, 2) }} </td>
                              <td style="text-align: center;">{{ round($data->quantity) }}</td>
                              <td style="text-align: center;">{{ round($data->weight,2) }}</td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($rs_per_pc, 2) }} </td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($rs_per_gm, 2) }} </td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($manpower_cost, 2) }} </td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($gw_cost, 2) }} </td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($total_manpower_cost, 2) }} </td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($total_manpower_cost_per_gm, 2) }} </td>
                              <td style="text-align: center;"> {{ $total_export_till_today_usd }} </td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($fg_value_rs_1000_per_pc, 2) }}</td>
                              <td style="text-align: center;"> {{ "₹ ".number_format($total_expenses_on_manpower, 2) }} </td>
                              <td style="text-align: center;"> {{  number_format($per_of_manpower_expenses_against_fg_value, 2) }} </td>
                            </tr>
